...
export function TransportProvider({ children }) {
  ...
  const addRoom = useCallback((r) => {
    const id = genId('rm');
    setRooms((rs) => rs.concat({ id, booked: 0, ...r }));
    return id;
  }, [genId]);
  const setRoomTotal = useCallback((id, total) => {
    let refused = null;
  ...
  const bookRoom = useCallback(({ roomId, checkIn, nights, guests, method, methodDetail, guestName }) => {
  ...
  const cancelRoomBooking = useCallback((ref) => {
  ...
  // --- menu ----------------------------------------------------------------
  const addMenuItem = useCallback((m) => {
    const id = genId('mn');
  ...
  const createLead = useCallback(({ kind, subjectId, subjectLabel, name, date, count, note }) => {
    const id = genId('ld');
  ...
  const sendQuote = useCallback((id, { lineItems, expiryHours }) => {
  ...
  const declineLead = useCallback((id) =>
    setLeads((ls) => ls.map((l) => (l.id === id ? { ...l, status: 'declined' } : l))), []);

  const withdrawQuote = useCallback((id) =>
  ...
  const previewLeadOutcome = useCallback((id, status) =>
  ...
  const acceptLead = useCallback((id) =>
